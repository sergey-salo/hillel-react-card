...
const ProductList = () => {
  ...
  const handleProductClick = (productId, fieldName) => {
    // setProducts(products.map((item) => (item.id === productId ? { ...item, [fieldName]: !item[fieldName] } : item)));

    const updatedProducts = products.map((item) =>
      item.id === productId ? { ...item, [fieldName]: !item[fieldName] } : item,
    );
    setProducts(updatedProducts);
  };

  const handleWishListClick = (productId) => handleProductClick(productId, "isInWishList");

  const handleCompareClick = (productId) => handleProductClick(productId, "isInCompareList");

  const handleAddToCartClick = (productId) => handleProductClick(productId, "isInCart");
